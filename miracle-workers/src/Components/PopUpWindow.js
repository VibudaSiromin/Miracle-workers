...
function ModalDialog(props) {
  ...
  const myLoop = () => {
      ...
      for(let i=0;i<props.noFields[0];i++){
        inputFieldArrayModalOne.push(<PopUpInputField title={props.title[i]} inputType="text"></PopUpInputField>);
        console.log('Bye');
      }
    if(props.enableChainPopUps){
      ...
      for(let i=0;i<props.noFields[1];i++){
        console.log('Hi');
        inputFieldArrayModalTwo.push(<PopUpInputField title={props.title[props.noFields[0]+i]} inputType="text"></PopUpInputField>);
      }  
    }
    ...
    <>
      ...
      <Modal show={toggleOneModal}  tabIndex='-1'>
       ...
       <Modal show={toggleTwoModal}  tabIndex='-1'>
        ...
        <Modal.Body>
          {inputFieldArrayModalTwo}
        </Modal.Body>
        <Modal.Footer>
          <Button variant="danger" onClick={TerminateModalTwo}>
            Close
          </Button>
          <Button variant="dark" onClick={TerminateModalTwo}>
            Finish
          </Button>
        </Modal.Footer>
      </Modal> 
