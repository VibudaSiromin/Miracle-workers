      ...
      <Modal show={toggleOneModal} tabIndex="-1" size="sm" centered>
        <form /*ref={ref}*/ onSubmit={submitHandlerOne} id={'renameForm'}>
          ...
          <Modal.Body>
            ...
            />
            {showErrMsgOne ? (
              <small className="text-danger">{'Enter a sheet name'}</small>
            ) : (
              <></>
            )}
          </Modal.Body>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    setTestPageName: (renamedPageName) =>
      dispatch(setTestPageName(renamedPageName)),
  };
  // forwardRef: true
};
