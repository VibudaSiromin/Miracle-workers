...
const Table = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      testSteps.length <= page[0] * rowsPerPage + rowsPerPage &&
      testSteps.length !== 0
    ) {
    ...
    if (props.removeHeading === true) {
      props.title.map((heading, headingIndex) => {
        tableFields.push(
          <th>
            {heading}
            <MdClose onClick={() => removeHeading(headingIndex)}></MdClose>
          </th>
        );
      });
    } else {
  ...
  return (
    <div className="App">
        ...
        <table id="data-Table">
          ...
          <tbody>
            {props.title.length !== 0 ? (
              testStepsPerPage.map((testStep, index) => (
                <Raw
                  testStep={testStep}
                  rawIndex={page[0] * rowsPerPage + index}
                  onDelete={deleteHandler}
                  onEdit={editHandler}
                  onArrowClick={arrowClickHandler}
                  title={props.title}
                  generalPurpose={props.generalPurpose}
                  enableChainPopUps={props.enableChainPopUps}
                />
              ))
            ) : (
              <div className="container no-record-msg">
                <TbTableOff size="100px"></TbTableOff>
      ...
      <Button onClick={jsonHandler}>Generate JSON</Button>
    </div>
  );
};
