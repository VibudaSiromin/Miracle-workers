...
const getReferedLocatorPages = async (req, res, next) => {
  const locatorPageName = req.query.locatorPageName;
  let testSection;
  try {
    const data = await fs.promises.readFile(testFilePath);
    testSection = JSON.parse(data);
    const testPages = testSection.map((testSheet) => {
      for (let i = 1; i < testSheet.length; i++) {
        console.log("Booommmmmmmmmm", testSheet)
        if ('locator' in testSheet[i] && testSheet[i]['locator'].charAt(0) === '#' && testSheet[i]['locator'].includes('.')) {
          const textSegments = testSheet[i]['locator'].split(".");
          if (textSegments[1] === locatorPageName) {
            return testSheet[0].slice(0, -1);
          }
        }
      }
    ...
    let filteredTestPages = testPages.filter(value => value !== undefined);
    console.log("BRAVOSIXXXXXXXXXXXXXXXXXXXXXx", testPages);
    res.status(200).json({ testPages: filteredTestPages })
  } catch (err) {
    console.log(err);
    res.status(500).json({ message: 'Error occurred when reading test section' });
  }
}
...
exports.getReferedLocatorPages = getReferedLocatorPages;